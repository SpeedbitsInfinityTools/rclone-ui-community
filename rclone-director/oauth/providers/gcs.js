/**
 * Google Cloud Storage OAuth Provider
 * 
 * Implements Google Cloud Storage-specific OAuth flow
 * Based on Rclone's googlecloudstorage.go implementation
 */
...
class GCSProvider extends BaseProvider {
    ...
    /**
     * Get authorization URL for Google Cloud Storage OAuth
     * 
    ...
    /**
     * Exchange authorization code for Google Cloud Storage access token
     * 
    ...
    /**
     * Format Google Cloud Storage token for Rclone config
     * Google Cloud Storage returns standard OAuth2 token format
     * 
     * @param {Object} tokenResponse - Token response from Google Cloud Storage
    ...
    /**
     * Refresh Google Cloud Storage access token using refresh token
     * 
    ...
    async refreshToken(refreshToken, clientId, clientSecret) {
        ...
        try {
            ...
            console.error('[Google Cloud Storage] Error refreshing token:', error.response?.data || error.message);
            throw new Error(`Failed to refresh Google Cloud Storage token: ${error.response?.data?.error_description || error.message}`);
        }
    }
    
    /**
     * Get Google Cloud Storage account information
     * 
    ...
    async getAccountInfo(accessToken, refreshToken = null, options = {}) {
        try {
            ...
            if (error.response?.status === 401 && refreshToken && options.clientId && options.clientSecret) {
                console.log('[Google Cloud Storage] Access token expired, refreshing...');
                try {
                    ...
                    console.error('[Google Cloud Storage] Error refreshing token:', refreshError.response?.data || refreshError.message);
                    throw new Error(`Failed to refresh expired token: ${refreshError.response?.data?.error_description || refreshError.message}`);
                }
            }
            
            console.error('[Google Cloud Storage] Error fetching account info:', error.response?.data || error.message);
            throw new Error(`Failed to get Google Cloud Storage account info: ${error.response?.data?.error_description || error.message}`);
        }
    }
}
