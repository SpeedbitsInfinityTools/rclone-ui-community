...
import {getAllRemoteNames, getRemoteInfo} from "rclone-api";
import {getFilesListWithBucketInfo} from "../utils/API/filesAPI";
...
export const getFsInfo = (remoteName) => dispatch => {
    ...
    getRemoteInfo(remoteName).then((res) => {
        ...
        error => {
            console.error('[EXPLORER] Failed to get remote info:', error);
            ...
            if (error.response?.status === 503 || error.code === 'ECONNREFUSED') {
                ...
                toast.error(
                    `Failed to get remote info: ${error.response?.data?.message || error.message}`,
                    { autoClose: 5000 }
                );
            }
...
export const getRemoteNames = () => {
    return (dispatch, getState) => {
        getAllRemoteNames().then(res => dispatch({
            ...
            console.error('[EXPLORER] Failed to get remote names:', error);
            ...
            if (error.response?.status === 503 || error.code === 'ECONNREFUSED') {
                ...
                toast.error(
                    `Failed to load remote list: ${error.response?.data?.message || error.message}`,
                    { autoClose: 5000 }
                );
            }
 ...
 * @returns {Function}
 */
export const getFiles = (remoteName, remotePath) => (dispatch, getState) => {
    if (remoteName !== "") {
        const path = `${remoteName}-${remotePath}`;
        
        // Get fsInfo from state for bucket detection fallback (older Rclone versions)
        const state = getState();
        const remoteNameOnly = remoteName.split(':')[0];
        const fsInfo = state.remote.configs && state.remote.configs[remoteNameOnly];
        
        getFilesListWithBucketInfo(remoteName, remotePath, fsInfo).then(res => dispatch({
                type: GET_FILES_LIST,
                status: REQUEST_SUCCESS,
                payload: {path: path, filesList: res.list}
            })
        ).catch(error => {
            console.error('[EXPLORER] Failed to get files:', error);
            ...
            if (error.response?.status === 503 || error.code === 'ECONNREFUSED') {
                ...
                toast.error(
                    `Failed to load files: ${error.response?.data?.message || error.message}`,
                    { autoClose: 5000 }
                );
            }
