 ...
 */
function explorerReducer(state = initialState, action) {
    switch (action.type) {
        ...
        case GET_REMOTE_LIST:
            if (action.status === REQUEST_SUCCESS) {
                // Filter out base remotes (internal implementation detail for encrypted remotes)
                const filteredRemotes = action.payload ? 
                    action.payload.filter(remoteName => !remoteName.endsWith('_base')) : 
                    [];
