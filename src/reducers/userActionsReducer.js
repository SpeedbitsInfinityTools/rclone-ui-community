...
const initialState = {
    auth: {authKey: "", ipAddress: "/api/rclone", interceptor: ""}
};
...
function userActionsReducer(state = initialState, action) {
    switch (action.type) {
        ...
        case CHANGE_IP_ADDRESS:
            ...
            sessionStorage.setItem(AUTH_KEY, state.auth.authKey);
            // sessionStorage.setItem(USER_NAME_KEY, state.auth.userName);
            // sessionStorage.setItem(PASSWORD_KEY, state.auth.password);
            sessionStorage.setItem(IP_ADDRESS_KEY, state.auth.ipAddress);
            
            // rclone-api library reads from localStorage, not sessionStorage
        ...
        case CHANGE_AXIOS_INTERCEPTOR:
            ...
            const interceptorID = axiosInstance.interceptors.request.use(
                config => {
                    ...
                    config.headers.Authorization = 'Basic ' + sessionStorage.getItem(AUTH_KEY);
                    // Set baseURL to our proxy route for both axiosInstance and rclone-api calls
