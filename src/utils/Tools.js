...
/**
 * Validate Size Suffix of the format (off | 1K | 1M | 100G | 10P | 48Mi | 10Gi) etc
 * Supports both decimal (K, M, G, T, P) and binary (Ki, Mi, Gi, Ti, Pi) units
 * @param str {string} String to be validated
 * @returns {boolean|*|never}
 */
export function validateSizeSuffix(str) {
    // Support both decimal units (K, M, G, T, P) and binary units (Ki, Mi, Gi, Ti, Pi)
    // Also support 'off' as a special value
    const regex = /^(off|(([0-9]+[.][0-9]+|[0-9]+)([KMGTP]i?)))$/i;
 ...
 * 
 * Rclone allows remote names with spaces and various characters, but:
 * - Cannot start or end with a space
 * - Cannot start with a hyphen (-)
 * - Should avoid characters that cause shell issues (but Rclone itself allows them)
 * 
 * For UI compatibility, we allow: letters, numbers, spaces, underscores, hyphens, dots, plus signs, at signs
 * But we enforce: no leading/trailing spaces, no leading hyphens
 * 
...
export function validateDriveName(name) {
    ...
    // Empty string is valid (allows deletion)
    if (name === '') {
        return true;
    }
    
    // Trim to check for leading/trailing spaces
    const trimmed = name.trim();
    if (trimmed !== name) {
        return false; // Has leading or trailing spaces
    }
...
export function findFromConfig(config, name) {
    // Defensive check: ensure config is an array
    if (!config || !Array.isArray(config) || config.length === 0) {
        console.warn('[findFromConfig] Config is not available or not an array:', config);
        return undefined;
    }
...
/**
 * Check if a remote configuration supports OAuth authentication.
 * OAuth remotes have both client_id and client_secret fields (usually optional).
 * @param config {$ObjMap} Array of remote configs
...
export function supportsOAuth(config, drivePrefix) {
    ...
    const remoteConfig = findFromConfig(config, drivePrefix);
    if (!remoteConfig || !remoteConfig.Options) {
        return false;
    }
