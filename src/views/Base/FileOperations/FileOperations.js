...
function getUrl(currentPath) {
	...
	// If remotePath already starts with '/', don't add another slash
...
class FileOperations extends React.Component {
	...
	openNewFolderModal = () => {
		// Always allow folder creation - we'll handle S3 with placeholder files
	...
	toggleClearTrashModal = () => {
		this.setState(prevState => ({
			showClearTrashModal: !prevState.showClearTrashModal
		}));
	};
	...
	handleConfirmClearTrash = async () => {
		const {currentPath, containerID} = this.props;
		let {remoteName} = currentPath;

		this.setState({ clearingTrash: true });

		try {
			const res = await cleanTrashForRemote(remoteName);
			if (res.status === 200) {
				// Close modal
				this.toggleClearTrashModal();
			...
			this.setState({ clearingTrash: false });
		}
	};
	...
	onSubmitUrlChange = (e) => {
		...
		if (urlSplits && urlSplits[0]) {
			let remoteName = urlSplits[0];
			let remotePath = urlSplits[1] || "";
			
			// Add leading slash if the path doesn't start with one and isn't empty
			if (remotePath && !remotePath.startsWith('/')) {
				remotePath = '/' + remotePath;
			}
			
			if (currentPath.remoteName !== remoteName || currentPath.remotePath !== remotePath) {
				changePath(containerID, remoteName, remotePath);
			}
		}
	}

	render() {
		const {containerID, getFilesForContainerID, gridMode, navigateFwd, navigateBack, navigateUp, searchQuery, currentPath, doughnutData} = this.props;
		const {newFolderModalIsVisible, dropdownOpen, isAboutModalOpen, searchOpen, tempUrl, isUrlBarFocused, showClearTrashModal, clearingTrash} = this.state;
		...
		return (
			<div className="pl-0 mt-3 mb-1 d-flex justify-content-between align-items-center"
				...
				<div className="d-flex flex-nowrap">
					...
					<Button color="light" className="mr-1 btn-explorer-action" id="NavigateUpButton"
							onClick={() => navigateUp(containerID)}><i
						className={"fa fa-lg fa-level-up"}/></Button>
					<UncontrolledTooltip placement="right" target="NavigateUpButton">
				...
				<div className="d-flex flex-wrap">
					...
					<FileUploadModal 
						...
						onUploadComplete={() => getFilesForContainerID(containerID)}
						buttonLabel={"Upload"}
					...
					<ConfirmModal
						isOpen={showClearTrashModal}
						toggle={this.toggleClearTrashModal}
						onConfirm={this.handleConfirmClearTrash}
						title="Clear Trash"
						message={
							<>
								<p>Are you sure you want to clear the trash for <strong>{remoteName}</strong>?</p>
...
const getFsInfoSelector = createSelector(
	...
	(containerID, currentPath, configs) => {
		...
		// Try to extract base remote name (without bucket) for S3
		// e.g., "hetzners3:bucket-name" -> "hetzners3"
		if (currentPath && currentPath.remoteName && configs) {
			const baseRemoteName = currentPath.remoteName.split(':')[0];
			if (configs[baseRemoteName]) {
				return configs[baseRemoteName];
			}
		}
