...
class FileComponent extends React.Component {
    ...
    * */
    ...
    render() {
        const {containerID, inViewport, item, loadImages, clickHandler, downloadHandle, linkShareHandle, deleteHandle, infoHandle, connectDragSource, gridMode, isDownloading, isSelected, toggleSelection /*isDragging, remoteName*/} = this.props;

        const {IsDir, MimeType, ModTime, Name, Size, ID} = item;
        ...
        if (gridMode === "card") {
                    ...
                    <Card>
                        <CardBody onClick={(e) => this.handleClick(IsDir, clickHandler, e, item)}>
                            ...
                            {loadImages && isMedia(MimeType) ?
                                ...
                                <FileIcon IsDir={IsDir} IsBucket={item.IsBucket} MimeType={MimeType}/>
                            }
                            {Name}
                        </CardBody>
                        <CardFooter>
                            <FileActions downloadHandle={downloadHandle} linkShareHandle={linkShareHandle}
                                         deleteHandle={deleteHandle} infoHandle={infoHandle} item={item}
            ...
            element = connectDragSource(
                <tr className="pointer-cursor fadeIn">
                    <td style={{width: '50px', textAlign: 'center', verticalAlign: 'middle', paddingLeft: '15px', position: 'relative'}} onClick={(e) => e.stopPropagation()}>
                        <Input 
                            type="checkbox" 
                            checked={isSelected || false}
                            onChange={() => toggleSelection && toggleSelection(fileId)}
                            style={{cursor: 'pointer', margin: '0', position: 'static'}}
                        />
                    </td>
                    <td onClick={(e) => clickHandler(e, item)}>
                        <FileIcon IsDir={IsDir} IsBucket={item.IsBucket} MimeType={MimeType}/>{" "}{Name}
                    </td>
                    ...
                    <td><FileActions downloadHandle={downloadHandle} linkShareHandle={linkShareHandle}
                                     deleteHandle={deleteHandle} infoHandle={infoHandle} item={item}
                                     isDownloading={isDownloading}/></td>
                </tr>
            )
        }
...
FileComponent.propTypes = {
    ...
    /**
     * Function to show info about a file/folder.
     */
    ...
    /**
     * Boolean value to represent if this file is currently being downloaded.
     */
