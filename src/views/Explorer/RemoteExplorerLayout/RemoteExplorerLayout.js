...
class RemoteExplorerLayout extends React.Component {
	...
	componentDidMount() {
		...
		this.serverChangeHandler = (event) => {
			console.log('[Explorer] Server changed, reloading explorer...');
			// Force reload of explorer by re-adding the container
			if (numContainers >= 1) {
				// Trigger a refresh by changing the layout
				this.changeLayout(numContainers, 'default');
			}
		};
	...
	handleRefresh = () => {
		// Trigger a manual refresh by dispatching the server-changed event
		const {numContainers} = this.props;
		this.setState({ isRefreshing: true });
		window.dispatchEvent(new CustomEvent('rclone-server-changed', { 
			detail: { manual: true } 
		}));
		// Also trigger layout refresh
		if (numContainers >= 1) {
			this.changeLayout(numContainers, 'default');
		}
					...
					<Row className={"d-none d-md-block"} data-test="remoteExplorerLayout">
                        ...
                        <span className="text-choose-layout">
							...
							<Button color={"secondary"} className={"ml-2"}
								...
								{isRefreshing ? (
									<><i className="fa fa-spinner fa-spin"/> Refreshing...</>
								) : (
